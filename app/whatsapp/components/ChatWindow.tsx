...
function MediaContent({ msg }: { msg: Message }) {
  ...
  if (msg.media_type === "document") {
    const filename = rawText.replace(/^\[document\]\s*/i, "").trim() || "Document";
    return (
      <div className="flex items-center gap-2">
        <svg className="w-8 h-8 text-[#8696a0] shrink-0" fill="currentColor" viewBox="0 0 24 24">
        ...
        {msg.media_url ? (
          ...
          <span className="text-sm text-[#667781]">{filename}</span>
        )}
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col bg-[#efeae2]">
      ...
      <div className="pl-2 pr-3 py-2 bg-[#f0f2f5] flex items-end gap-2 shrink-0">
        ...
        <form onSubmit={handleSend} className="flex-1 flex items-center bg-white rounded-3xl px-3 py-1 gap-1 shadow-sm min-w-0">
          ...
          {/* TODO: paperclip (PDF) hidden until Android pick-bug is resolved */}
        </form>
